...
path_a = [0.250, 0.000, 0.000, 0.000]
path_b = [0.458, 0.125, 0.500, 0.187]
path_c = [0.574, 0.750, 0.667, 0.644]
...
for i, w in enumerate(winners):
    ...
    ax.annotate(
        "★",
        xy=(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.030),
        ha="center", va="bottom",
...
ax.annotate(
    "★ = highest metric score (Path C wins all)",
    xy=(0.98, 0.93), xycoords="axes fraction",
    ha="right", va="top", fontsize=8, color="#555555",
    bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="#cccccc", alpha=0.8)
)

# highlight the Context Recall result — the key finding
ax.annotate(
    "Path A: 0.000\n(complete failure)",
    xy=(x[1] - width, 0.005),
    xytext=(x[1] - width - 0.45, 0.18),
    textcoords="data",
    fontsize=7.5, color="#4C72B0",
    arrowprops=dict(arrowstyle="->", color="#4C72B0", lw=1.2),
    bbox=dict(boxstyle="round,pad=0.3", fc="#EEF3FA", ec="#4C72B0", alpha=0.9)
)

plt.title("RISK Questions (n=2): RAGAS Scores by Pipeline", fontsize=12.5, pad=12)
plt.tight_layout()

out_path = "results/reports/risk_ragas_bar.png"
